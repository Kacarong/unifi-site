...
전사는 세 가지 중 하나로 넣는다. 셋 다 같은 자리로 들어간다.

1. **위에서 영상을 추출했으면 자동으로 물린다.** 추출이 끝나면 "이 전사로 정리본
   만들기" 버튼이 뜨고, 누르면 아래로 내려가며 전사가 연결된다. 연결되면 직접
   넣는 칸은 접힌다 — 칸이 그대로 보이면 "여기에 또 붙여넣으라는 건가" 로 읽힌다.
   다시 펴려면 "전사 직접 넣을래요" 를 누른다. 예전에는 화면 어디에도 보여 주지
   않는 '작업 번호'를 손으로 옮겨 적게 해 뒀다. 옮길 이유가 없다.
2. **전사 파일(.txt/.srt/.vtt)을 올린다.**
3. **직접 붙여넣는다.**

강의자료 PDF 를 **같이** 넣으면 강사가 말한 것과 자료에 적힌 것을 제목으로 맞춰
한 섹션으로 묶는다. 그래서 목차 한 줄에 쪽수와 타임코드가 같이 붙는다
(`2장. 함수의 극한 (p.2, 00:00:06)`). 둘 중 하나만 넣어도 동작한다.
